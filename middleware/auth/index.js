...
const apiAuthMiddleware = (req, res, next) => {
    ...
    if (!authHeader) {
        return res.status(401).json({status: "auth_failed", message: 'Authorization header missing' });
    }

    const token = authHeader.split(' ')[1];
    if (!token) {
        return res.status(401).json({status: "auth_failed", message: 'Authentication API KEY missing' });
    }
    ...
    try {
        ...
        if(decodedToken.type === 'api_token') {
            ...
            ApiTokenModel.findOne({where : {token}}).then( tokenObj => {
